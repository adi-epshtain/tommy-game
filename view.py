...
async def start_game(player_name: str):
    if player_name not in games:
        games[player_name] = await Game.create(player_name=player_name)
    question = await games[player_name].get_question()
...
async def submit_answer(player_name: str, req: AnswerRequest):
    if player_name not in games:
        raise HTTPException(status_code=404, detail="Player not found")
    game = games[player_name]
    is_correct: bool = await game.submit_answer(req.answer)
...
async def end_game(request: Request, player_name: str):
    score = games[player_name].player.score
