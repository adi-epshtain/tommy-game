...
from dal.player_session_dal import create_player_session, \
    update_player_session, end_session
...
async def start_game(player_name: str, db: Session = Depends(get_db)):
    player: Player = get_player_by_name(db, player_name)
    if not player:
        player = create_player(db, name=player_name, age=5)  # TODO: add age

    game: Game = get_game_by_name(db, GameName.MATH_GAME.value)
    if not game:
        game = create_game(db, name=GameName.MATH_GAME.value,
                           description="Default")

    player_session: PlayerSession = create_player_session(db, player_id=player.id,
                                           game_id=game.id)
...
async def submit_answer(req: AnswerRequest, db: Session = Depends(get_db)):
    player: Player = get_player_by_name(db, req.player_name)
    if not player:
        raise HTTPException(status_code=404, detail="Player not found")

    question: Question = get_question_by_id(db, req.question_id)
    if not question:
        raise HTTPException(status_code=404,
                            detail=f"Question not found question id:"
                                   f" {req.question_id}")

    player_session: PlayerSession = db.query(PlayerSession).filter(
    ...
    game: Game = get_game_by_name(db, req.game_name)
