...
def create_player(session: Session, name: str, age: int) -> Optional[Player]:
    try:
...
def get_player_by_name(session: Session, player_name: str) -> Optional[Player]:
    player = session.query(Player).filter(Player.name == player_name).first() or None
    return player


def delete_player(session: Session, player_id: int) -> Optional[Player]:
